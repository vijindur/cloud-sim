...
LEGACY_ALGOS = ["ROUND_ROBIN", "FCFS", "FIRST_FIT", "BEST_FIT"]
...
def load_results() -> pd.DataFrame:
    ...
    aggregated = df.groupby("algorithm", as_index=False).agg(
        {**{column: "mean" for column in numeric_cols}, "workload": "first"}
    )
    return aggregated.set_index("algorithm")
...
def simulate_algorithm_metrics(
    ...
    for algorithm in algorithms:
        if algorithm not in results_lookup.index:
            continue

        profile = ALGO_PROFILES.get(algorithm, ALGO_PROFILES["FIRST_FIT"])
...
all_available_algorithms = list(dict.fromkeys(LEGACY_ALGOS + MODERN_ALGOS))
algorithms = st.sidebar.multiselect(
    "Algorithms in comparison",
    all_available_algorithms,
    default=MODERN_ALGOS + [algo for algo in LEGACY_ALGOS if algo in results_lookup.index.tolist()],
)
...
selected_focus = st.sidebar.selectbox("Highlight optimized algorithm", focus_candidates, index=0 if "HYBRID" not in focus_candidates else focus_candidates.index("HYBRID"))
playback_algorithm = st.sidebar.selectbox("Playback algorithm", algorithms, index=algorithms.index(selected_focus) if selected_focus in algorithms else 0)
...
measured_algos = results_lookup.index.tolist()
if metric_mode == "Measured (experiment CSV)" and selected_focus not in measured_algos:
    use_projection = True
    st.info(f"📊 **{selected_focus}** is showcased using projection mode (since measured results are not available yet).")
else:
    use_projection = metric_mode == "Projection (what-if)"
...
if selected_focus == "HYBRID":
    st.success("⭐ **HYBRID** combines heuristic speed with PSO optimization for best-in-class efficiency and latency.", icon="✨")

if use_projection and selected_focus in MODERN_ALGOS and selected_focus not in measured_algos:
    st.info(f"📊 **{selected_focus}** is showcased using projection mode (since measured results are not available yet).")
elif metric_mode != "Projection (what-if)":
    st.info(
...
with overview_tab:
    ...
    with right:
        ...
        def get_bar_color(algo):
            if algo == selected_focus:
                return "#163f3c"  # Dark green for selected
            elif algo == "HYBRID":
                return "#e07a3f"  # Accent orange for HYBRID
            else:
                return "#d9b08c"  # Neutral tan for others
        
        sns.barplot(
            data=summary_df,
            y="algorithm",
            x="compositeScore",
            palette=[get_bar_color(algo) for algo in summary_df["algorithm"]],
            ax=ax,
        )
...
with playback_tab:
    ...
    st.markdown(
        ...
        <div class="glass-card">
            <div class="section-copy">
                <strong>{playback_algorithm}</strong> uses the strategy <strong>{ALGO_PROFILES.get(playback_algorithm, ALGO_PROFILES['FIRST_FIT'])['label']}</strong>.
